...
| --- | --- | --- |
| Efficiency and throughput | [`efficiency.md`](efficiency.md) | Matched Sparse-vLLM/vLLM length, concurrency, churn, TP, latency, throughput, and directly sampled GPU-activity suite. |
| Sparse-vLLM microbenchmark | `benchmark/microbench.py` | Synthetic prompt-length engine throughput, TTFT, prefill/decode throughput, ITL, and GPU memory. |
...
| LongBench | `benchmark/long_bench/pred.py`, `benchmark/long_bench/eval.py` | Native Sparse-vLLM runner; use `--task` for subsets and omit it for the full suite. |
| MathBench, AIME, MATH-500 | `benchmark/math_bench/pred.py`, `benchmark/math_bench/eval.py` | Native Sparse-vLLM runner for `gsm8k`, `aime2024`, `math500`, and `hmmt_nov`. |
...
| Claw-Eval | `benchmark/claw_eval/run_sparsevllm_claw_eval.sh` | External Claw-Eval checkout driven through the shared Sparse-vLLM OpenAI-compatible server. |
...
| RULER-VT | `benchmark/ruler_vt/pred.py` | Self-contained RULER variable-tracking evaluator using the native benchmark adapter. |
| NIAH | `benchmark/niah/test_niah.py` | Native Sparse-vLLM needle-in-a-haystack long-context utility runner. |
| Regression harness | [`sparsevllm-regression-tests.md`](sparsevllm-regression-tests.md) | Fixed quality/performance/stress checks for engine regressions. |
...
The old `scripts/benchmarks/bench_sparse_vllm.py` path remains as a
...
This entrypoint runs LongBench through the native Sparse-vLLM engine.
...
- SCBench uses the native `sparsevllm` attention type for Sparse-vLLM methods.
...
`sparsevllm.entrypoints.openai.api_server`, validates the Docker sandbox,
