...
| --- | --- | --- |
| `engine_prefill_chunk_size` | Sparse-vLLM | Max prefill chunk scheduled per sequence per step for `all_chunked`, and max RawKV offload chunk for `long_bs1full_short_batch`. Under the long policy it defaults to `long_prefill_offload_threshold`; an explicit value must satisfy `0 < engine_prefill_chunk_size <= long_prefill_offload_threshold`. |
...
| `long_prefill_offload_threshold` | Sparse-vLLM | Residual-token boundary between atomic batched full prefill and isolated chunked RawKV offload under `long_bs1full_short_batch`. Defaults to `65536` tokens (64K). |
...
| `engine_prefill_chunk_size` / `chunk_prefill_size` | `8192` | Max chunk for `all_chunked`. Under `long_bs1full_short_batch`, an omitted value defaults to the offload threshold and an explicit value may be smaller. Use the semantic `--engine-prefill-chunk-size` on the CLI. |
| `long_prefill_offload_threshold` | `65536` | Residual boundary for `long_bs1full_short_batch`; requests above it use isolated RawKV offload. |
