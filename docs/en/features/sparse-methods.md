...
which preserves the normalized softmax-probability score definition. Explicit
`logits` mode instead ranks tokens by the maximum raw QK value over observation
queries and query heads. Both modes use the shared Triton window scorer for GQA
and reconstructed MLA prefill. Logit scores require FP32 storage and never
fall back to a different score definition. H2O normalizes each max-logit token
vector and weights it by the number of observation queries before cumulative
accumulation. `h2o_prefill_score_window=0` selects the full current chunk only
in logits mode; probability mode remains limited to windows in `[1, 128]`.
