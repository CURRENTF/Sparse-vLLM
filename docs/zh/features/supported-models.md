...
| Qwen3.6 MoE | `qwen3_5_moe` | 仅 BF16 | ✅ | 仅支持 1 | ✅ |
...
`DP=1`、`T % E == 0` 且语言模型权重为 BF16。首版仅支持纯文本 CausalLM，
明确拒绝 image/video 与 MTP 输入，只支持 Vanilla KV runtime；CUDA Graph
仅覆盖 decode，不覆盖 prefill。
