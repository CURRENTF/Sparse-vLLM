...
class DecodeCudaGraphCapacityPolicyTest(unittest.TestCase):
    ...
    def test_piecewise_graph_bypasses_force_eager_guard(self):
        ...
        def run_piecewise(input_ids, positions, state, needs_capture, force_capture_alignment):
            del positions, state
            calls.append(f"piecewise:{needs_capture}:{force_capture_alignment}")
        ...
        self.assertEqual(calls, ["context:1024", "context:1024", "piecewise:True:False"])

    def test_piecewise_graph_propagates_capture_alignment(self):
        runner = self.make_runner("vanilla")
        calls = []
        runner.cache_manager = SimpleNamespace(
            device=torch.device("cpu"),
            select_decode_cuda_graph_batch_size=lambda real_size, sizes: real_size,
            set_decode_static_max_context_len=lambda value: None,
            prepare_decode_static=lambda seqs, input_ids, positions, slot_mapping, context_lens, req_indices: (
                input_ids,
                positions,
                None,
        ...
        def run_piecewise(input_ids, positions, state, needs_capture, force_capture_alignment):
            del input_ids, positions, state
            calls.append((bool(needs_capture), bool(force_capture_alignment)))
        ...
        runner.run(seqs, force_piecewise_capture_alignment=True)
