...
from sparsevllm.kernels.tilelang.mla.runtime import (
    TileMlaDecodeKernel, select_tile_mla_config,
)
...
def profiles(case, config, sm_count):
    old = replace(select_tile_mla_config(batch_size=case["batch"], need_score=True,
                                        local_q_heads=case["heads"]), score_mode="per_head")
