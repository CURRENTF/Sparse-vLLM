...
def main():
    ...
    missing = []
    for lane, label in [('hisparse-quest', 'HiSparse'), ('svllm-quest', 'Ours QuEST'),
                        ('tangram-snapkv', 'Tangram'), ('svllm-snapkv', 'Ours SnapKV')]:
    ...
    for i, (model, prefix) in enumerate([('Qwen3-4B', 'qwen4'), ('GLM-4.7-Flash', 'glm')]):
        ...
        table(axes[i, 1], rows)
    handles, labels = axes[0, 0].get_legend_handles_labels()
    fig.legend(handles, [s.replace('Sparse-vLLM', 'Ours').replace('wave2', 'Max Avai. B') for s in labels],
