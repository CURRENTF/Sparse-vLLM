...
Sparse-vLLM continues to use `benchmark/efficiency/bench_probe.py`. Vortex uses
its own canonical probe. All decode-window arithmetic is owned by Sparse-vLLM's
...
```bash
conda run -p "$NATIVE_ENV" python scripts/official_experiments/sparsevllm_vs_vortex/plot.py \
...
```bash
python3 scripts/official_experiments/sparsevllm_vs_vortex/prepare_runs.py \
...
```bash
python3 scripts/official_experiments/sparsevllm_vs_vortex/run_queue.py \
...
```bash
python3 scripts/official_experiments/sparsevllm_vs_vortex/summarize.py \
  --root "$RUN_ROOT" --output "$RUN_ROOT/summary_rebuilt.json"
python3 scripts/official_experiments/sparsevllm_vs_vortex/export_plot_data.py \
  --summary "$RUN_ROOT/summary_rebuilt.json" \
  --template scripts/official_experiments/sparsevllm_vs_vortex/data/decode.json \
