...
class GeluTanhAndMulSpec:
    ...
    def __post_init__(self) -> None:
        if int(self.input_ndim) <= 0:
            raise ValueError("GeluTanhAndMul input_ndim must be positive.")
...
GELU_TANH_AND_MUL_REGISTRY: OpRegistry[
    GeluTanhAndMulSpec, GeluTanhAndMulProvider
] = OpRegistry("GELU-tanh-and-multiply")
...
@GELU_TANH_AND_MUL_REGISTRY.register
class TritonGeluTanhAndMulProvider(GeluTanhAndMulProvider):
    name = "triton"
    priority = 10

    def __init__(self, *, op_spec: GeluTanhAndMulSpec) -> None:
        self.spec = op_spec

    @classmethod
    def supports(cls, spec: GeluTanhAndMulSpec, caps: DeviceCaps) -> SupportResult:
        if caps.platform != PlatformEnum.CUDA:
            return SupportResult.no(f"requires CUDA, got {caps.platform.name}")
        if not caps.supports_triton:
            return SupportResult.no("platform does not support Triton")
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        _validate_bound_input(x, self.spec)
        if not x.is_cuda:
            raise ValueError("Triton GeluTanhAndMul provider requires a CUDA input.")
...
@GELU_TANH_AND_MUL_REGISTRY.register
class TorchGeluTanhAndMulProvider(GeluTanhAndMulProvider):
    name = "torch"
    priority = 0

    def __init__(self, *, op_spec: GeluTanhAndMulSpec | None = None) -> None:
        self.spec = op_spec
...
def resolve_gelu_tanh_and_mul_provider(
    *,
    activation_dtype: torch.dtype,
    input_ndim: int = 2,
    contiguous: bool = True,
    ...
    if device_index is None:
        device_index = torch.cuda.current_device() if platform.is_cuda_alike() else 0
    caps = platform.get_device_caps(int(device_index))
    spec = GeluTanhAndMulSpec(
        activation_dtype=activation_dtype,
        input_ndim=int(input_ndim),
        contiguous=bool(contiguous),
    )
    return OpResolver(GELU_TANH_AND_MUL_REGISTRY).resolve(
        spec,
        caps,
        op_spec=spec,
    ).provider
