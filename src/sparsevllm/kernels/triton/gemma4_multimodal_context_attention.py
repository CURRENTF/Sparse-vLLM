...
@triton.jit
def _gemma4_multimodal_context_attention_kernel(
    q,
    k,
    v,
    output,
    ...
    query_groups = tl.load(
        image_groups + query_start + query_positions,
        mask=query_positions < query_len,
        other=0,
    )
    max_logit = tl.full((BLOCK_M,), -float("inf"), tl.float32)
    denominator = tl.zeros((BLOCK_M,), tl.float32)
    accumulator = tl.zeros((BLOCK_M, HEAD_DIM), tl.float32)
    max_key = prefix_len + query_len
    for key_start in range(0, max_key, BLOCK_N):
        ...
        key_relative = key_positions - prefix_len
        key_groups = tl.load(
            image_groups + query_start + key_relative,
            mask=(key_relative >= 0) & (key_relative < query_len),
            other=0,
        )
        same_image = (query_groups[:, None] > 0) & (
            query_groups[:, None] == key_groups[None, :]
        )
        visible = (key_positions[None, :] <= absolute_queries) | same_image
        if WINDOW > 0:
            visible &= key_positions[None, :] > absolute_queries - WINDOW
        visible &= key_positions[None, :] < max_key
        if SCORE_MODE == 3:
            score = tl.sum(
                tl.where(visible, logits * 0.6931471805599453, 0.0), axis=0
            )
            tl.atomic_add(
                attn_score
                + batch * stride_asb
...
@torch.no_grad()
def gemma4_multimodal_context_attention(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    output: torch.Tensor,
    ...
    *,
    ...
    if q.ndim != 3 or k.ndim != 3 or v.shape != k.shape or output.shape != q.shape:
        raise ValueError(
            "Gemma 4 multimodal attention requires matching rank-3 Q/K/V/output."
        )
    if head_dim not in {256, 512} or k.shape[-1] != head_dim:
        raise ValueError(
            f"Gemma 4 multimodal attention requires head_dim 256 or 512, got {head_dim}."
        )
    if not all(tensor.is_cuda for tensor in (q, k, v, output, image_groups)):
        raise TypeError("Gemma 4 multimodal attention requires CUDA tensors.")
    if q.dtype not in {torch.float16, torch.bfloat16} or any(
        tensor.dtype != q.dtype for tensor in (k, v, output)
    ):
        raise TypeError(
            "Gemma 4 multimodal attention requires matching FP16 or BF16 tensors."
        )
    if any(tensor.stride(-1) != 1 for tensor in (q, k, v, output)):
        raise ValueError(
            "Gemma 4 multimodal attention requires contiguous head dimensions."
        )
    if int(q.shape[1]) % int(k.shape[1]):
        raise ValueError("Gemma 4 multimodal attention requires divisible Q and KV heads.")
    if image_groups.shape != (q.shape[0],):
        raise ValueError(
            "Gemma 4 image groups must align with flattened queries: "
            f"groups={tuple(image_groups.shape)} q={tuple(q.shape)}."
        )
    if image_groups.dtype != torch.int32:
        raise TypeError("Gemma 4 image groups must use int32.")
    if int(sliding_window) <= 0:
        raise ValueError(f"Gemma 4 multimodal sliding window must be positive, got {sliding_window}.")
    if attn_score is not None and attn_score.dim() not in {2, 3}:
        raise ValueError(
            "Gemma 4 multimodal prefill scores must be [B, L] or [B, H, L], "
    ...
    _gemma4_multimodal_context_attention_kernel[
...
__all__ = ["gemma4_multimodal_context_attention"]
