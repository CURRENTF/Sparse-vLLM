...
def _get_ref_ratio(config: KVLlamaConfig) -> float:
...
def _get_compressed_keep_ratio(config: KVLlamaConfig, head_dim: int) -> Union[int, float]:
...
class LlamaDeltaSnapKVAttention(LlamaAttention):
...
class LlamaDeltaSnapKVLayer(LlamaDecoderLayer):
    def __init__(self, config: KVLlamaConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = LlamaDeltaSnapKVAttention(config=config, layer_idx=layer_idx)


class LlamaDeltaSnapKVModel(LlamaModel):
    def __init__(self, config: KVLlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDeltaSnapKVLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.config = config
        pprint(config)
        self.post_init()
...
class LlamaDeltaSnapKVForCausalLM(LlamaForCausalLM):
    def __init__(self, config: KVLlamaConfig):
        super().__init__(config)
        self.model = LlamaDeltaSnapKVModel(config)
        self.config = config
        self.post_init()
