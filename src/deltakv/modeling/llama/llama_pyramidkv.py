...
class LlamaPyramidKVAttention(LlamaAttention):
    def __init__(self, config: KVLlamaConfig, layer_idx: int):
        ...
        log_once(f"PyramidKV Llama Layer Ratios: {', '.join(all_ratios)}", "INFO")
...
class LlamaPyramidKVLayer(LlamaDecoderLayer):
    def __init__(self, config: KVLlamaConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = LlamaPyramidKVAttention(config=config, layer_idx=layer_idx)


class LlamaPyramidKVModel(LlamaModel):
    def __init__(self, config: KVLlamaConfig):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [LlamaPyramidKVLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.post_init()


class LlamaPyramidKVForCausalLM(LlamaForCausalLM):
    def __init__(self, config: KVLlamaConfig):
        super().__init__(config)
        self.model = LlamaPyramidKVModel(config)
        self.config = config
        self.post_init()
