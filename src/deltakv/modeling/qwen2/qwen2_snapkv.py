...
class Qwen2SnapKVAttention(Qwen2Attention):
...
class Qwen2SnapKVLayer(Qwen2DecoderLayer):
    def __init__(self, config: KVQwen2Config, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = Qwen2SnapKVAttention(config=config, layer_idx=layer_idx)


class Qwen2SnapKVModel(Qwen2Model):
    def __init__(self, config: KVQwen2Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [Qwen2SnapKVLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.post_init()


class Qwen2SnapKVForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config: KVQwen2Config):
        super().__init__(config)
        self.model = Qwen2SnapKVModel(config)
        self.config = config
        self.post_init()
    ...
    def forward(
        ...
        # 逻辑：将 prompt 切块，强制最后一个块的大小为 observation window size
        # 这样能保证在最后一个 chunk forward 时，SnapKVCache 知道这是 "last chunk"
        if seq_len > 1:
