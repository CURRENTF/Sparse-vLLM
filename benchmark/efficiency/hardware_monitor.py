...
"""Coarse nvidia-smi GPU utilization timeline and idle-duty analyzer."""
...
class GPUHardwareMonitor:
    ...
    def _monitor_loop(self) -> None:
        ...
        while not self._stop_event.is_set() and not _STOP:
            ...
            try:
                ...
                observed = {
                    idx for idx in self.gpus if f"gpu{idx}_util" in row
                }
                if observed != set(self.gpus):
                    raise RuntimeError(
                        f"nvidia-smi sample omitted requested GPUs: "
                        f"expected={self.gpus}, observed={sorted(observed)}"
                    )
                self.samples.append(row)
            except Exception as exc:
                self.collection_error_count += 1
                self.last_collection_error = repr(exc)
    ...
    def analyze_and_save(self) -> dict[str, Any]:
        ...
        if not self.samples:
            summary["status"] = "metric_failed"
            summary["error"] = (
                "No complete nvidia-smi samples were collected; coarse GPU duty metrics "
                "are unavailable."
            )
            self._save(summary)
            return summary
        if self.collection_error_count:
            summary["status"] = "metric_failed"
            summary["error"] = (
                f"{self.collection_error_count} nvidia-smi samples failed; "
                "the utilization timeline is incomplete."
            )
        ...
        for idx in self.gpus:
            utils = [s[f"gpu{idx}_util"] for s in self.samples]
            mem_utils = [s[f"gpu{idx}_mem_util"] for s in self.samples]
            powers = [s[f"gpu{idx}_power_w"] for s in self.samples]
            mems = [s[f"gpu{idx}_mem_mb"] for s in self.samples]
            ...
            # Coarse sampled duty: this cannot attribute idle time to host launch gaps.
            ...
            summary["gpus"][f"gpu_{idx}"] = {
                ...
                "avg_memory_io_activity_pct": round(avg_mem_util, 2),
                "avg_power_w": round(avg_power, 2),
                "max_power_w": round(max_power, 2),
                "energy_joules": round(energy_j, 2),
                "peak_vram_gb": round(peak_mem_gb, 2),
                "coarse_gpu_active_duty_pct": round(busy_pct, 2),
                "coarse_gpu_idle_duty_pct": round(idle_pct, 2),
        ...
        if self.gpus:
            ...
            mean_duty_cycle = sum(summary["gpus"][f"gpu_{idx}"]["coarse_gpu_active_duty_pct"] for idx in self.gpus) / n_gpus
            mean_idle_duty = sum(summary["gpus"][f"gpu_{idx}"]["coarse_gpu_idle_duty_pct"] for idx in self.gpus) / n_gpus

            summary["aggregate"] = {
                "num_gpus": n_gpus,
                "mean_compute_util_pct": round(avg_all_util, 2),
                "mean_memory_io_activity_pct": round(avg_all_mem_util, 2),
                "avg_total_power_w": round(avg_total_power, 2),
                "total_energy_joules": round(total_energy_joules, 2),
                "mean_coarse_gpu_active_duty_pct": round(mean_duty_cycle, 2),
                "mean_coarse_gpu_idle_duty_pct": round(mean_idle_duty, 2),
            }
...
def parse_args():
    parser = argparse.ArgumentParser(description="Coarse nvidia-smi GPU hardware timeline monitor.")
...
def main():
    ...
    if not target_gpus:
        raise ValueError("--gpus must name at least one GPU index.")
    ...
    print(f"[Hardware Monitor] Started coarse sampling of GPUs {target_gpus} every {args.interval_ms}ms...")
    monitor.start()
    ...
    try:
        ...
        for idx in target_gpus:
            ...
            print(
                f"  • Coarse GPU Duty: active={g_data.get('coarse_gpu_active_duty_pct', 0):5.1f}% "
                f"idle={g_data.get('coarse_gpu_idle_duty_pct', 0):5.1f}%"
            )
            print(f"  • Memory I/O Activity: Avg = {g_data.get('avg_memory_io_activity_pct', 0):5.1f}% | Peak VRAM = {g_data.get('peak_vram_gb', 0):.2f} GB")
