...
- `/v1/responses` 支持非流式文本输入、text-only message input、
  ...
  Qwen3 thinking 开关的映射、usage 和 incomplete 状态组装。
- 未实现完整语义的 Responses 控制字段会显式失败：`tool_choice` 仅接受
  `null`/`"auto"`，`parallel_tool_calls=false` 和 `reasoning.summary` 暂不支持。
- Responses streaming 第一版未实现，`stream=true` 显式返回错误，不复用
  Chat Completions chunk 格式。
- `--reasoning-parser qwen3` 支持把 `<think>...</think>` 拆成 Sparse-vLLM
  自有扩展 reasoning item 和 message/function_call item；parser 未启用时原样
  返回 `output_text`。
...
- Responses SSE 事件和流式 reasoning 状态机。
...
启动前必须先检查 GPU 是否空闲，选择空闲设备；如果所有设备都忙，先等待，
等待过长再报告情况，不要直接抢占忙碌设备。

示例启动命令可按实际空闲 GPU 和显存调整：
...
Responses stream 不应复用 chat chunk 格式。第一版如果实现，应在
...
如果事件语义还没有完整实现，`stream=true` 可以先返回 400。不要用错误的
chat chunk 格式冒充 Responses stream。
...
第一版如果不实现 Responses streaming，就不需要做流式 parser。
如果实现，必须覆盖标签跨 chunk 被切开的情况。
...
如果 worker 第一版尚未实现 Responses streaming，则 worker 返回 400；
router 只透传该错误，不伪造 Responses stream。
...
启动前必须按仓库规则检查 GPU 空闲状态，选择空闲设备；不要在忙碌设备上直接
启动验证任务。
